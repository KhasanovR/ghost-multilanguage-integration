...
def new_post():
    ...
    Translation.create(post_id=str(post["id"]), post_slug=str(post["slug"]), post_lang=src_lang, is_published=1, o_post=str(post["id"]))
    translations = send_post(post, src_lang)
    for trans in translations:
        lang = trans['lang']
        trans = trans['data']['posts'][0]
        Translation.create(post_id=str(trans["id"]), post_slug=str(trans["slug"]), is_published=0, post_lang=lang, o_post=str(post["id"]))
    db.commit()
    db.close()
